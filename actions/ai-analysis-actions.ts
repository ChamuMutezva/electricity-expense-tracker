...
import { getElectricityReadings, getTokenPurchases, getUsageSummary } from "./electricity-actions"
...
function formatDataForAI(readings: ElectricityReading[], tokens: TokenPurchase[], summary: any) {
  // Group readings by day and period
  const dailyData = readings.reduce(
    (acc, reading) => {
  ...
  const consumptionData = Object.entries(dailyData)
    .map(([date, periods]) => {
      ...
      const morningConsumption = morning > 0 && evening > 0 ? morning - evening : 0
      const eveningConsumption = evening > 0 && night > 0 ? evening - night : 0
...
/**
 * Generate AI insights about electricity usage patterns using DeepSeek
 */
export async function generateElectricityInsights(): Promise<string> {
  try {
    console.log("Starting AI insights generation with DeepSeek...")
    ...
    console.log(`Data retrieved: ${readings.length} readings, ${tokens.length} tokens`)
...
Keep recommendations practical and specific to this usage data.`,
    ...
    console.log("AI insights generated successfully with DeepSeek")
    return text
  } catch (error) {
    ...
    if (error instanceof Error) {
      if (error.message.includes("API key")) {
        return "❌ **Configuration Error**: DeepSeek API key is not configured. Please add your DeepSeek API key to the environment variables."
      }
      if (error.message.includes("quota")) {
        return "❌ **Quota Exceeded**: You've exceeded your DeepSeek API quota. Please check your DeepSeek account."
      }
...
/**
 * Chat with AI about electricity data using DeepSeek
 */
export async function chatAboutElectricity(question: string, conversationHistory: string[] = []) {
  try {
    console.log("Starting AI chat with DeepSeek...")
    ...
    console.log(`Chat data: ${readings.length} readings, ${tokens.length} tokens`)
...
${JSON.stringify(analysisData, null, 2)}`,
    ...
    console.log("AI chat response generated successfully with DeepSeek")
    ...
    if (error instanceof Error) {
      if (error.message.includes("API key")) {
        throw new Error(
          "DeepSeek API key is not configured. Please add your DeepSeek API key to the environment variables.",
        )
      }
      if (error.message.includes("quota")) {
        throw new Error("You've exceeded your DeepSeek API quota. Please check your DeepSeek account.")
      }
      if (error.message.includes("rate limit")) {
