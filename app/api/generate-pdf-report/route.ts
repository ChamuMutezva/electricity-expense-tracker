...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!month || !readings) {
      return NextResponse.json({ error: "Month and readings data are required" }, { status: 400 })
    }

    // Filter data for the selected month
    const [year, monthNum] = month.split("-")
    const monthReadings = readings.filter((reading: any) => {
      const date = new Date(reading.timestamp)
      return date.getFullYear() === Number(year) && date.getMonth() === Number(monthNum) - 1
    })
    ...
    doc.text("Electricity Usage Report", pageWidth / 2, 30, { align: "center" })

    doc.setFontSize(14)
    doc.setFont("helvetica", "normal")
    const monthName = new Date(Number(year), Number(monthNum) - 1, 1).toLocaleDateString(undefined, {
      year: "numeric",
      month: "long",
    })
    ...
    const totalTokens = monthTokens.reduce((sum: number, token: any) => sum + token.units, 0)

    let totalUsage = 0
    for (let i = 1; i < monthReadings.length; i++) {
      const usage = monthReadings[i - 1].reading - monthReadings[i].reading
      if (usage > 0) totalUsage += usage
    }

    const avgDailyUsage = totalUsage / Math.max(1, new Date(Number(year), Number(monthNum), 0).getDate())

    doc.text(`Total Readings: ${totalReadings}`, 20, yPosition)
    yPosition += 10
    doc.text(`Total Usage: ${totalUsage.toFixed(2)} kWh`, 20, yPosition)
    yPosition += 10
    doc.text(`Average Daily Usage: ${avgDailyUsage.toFixed(2)} kWh`, 20, yPosition)
    yPosition += 10
    doc.text(`Tokens Purchased: ${totalTokens.toFixed(2)} kWh`, 20, yPosition)
    ...
    if (monthTokens.length > 0) {
      ...
      monthTokens.forEach((token: any) => {
        ...
        doc.text(token.total_cost ? `$${token.total_cost.toFixed(2)}` : "N/A", 140, yPosition)
        yPosition += 8
      })
    }
