...
export default function ElectricityTracker({
  ...
  function setupNotifications(setNotificationsEnabled: (enabled: boolean) => void) {
  ...
  function checkMigration(dbConnected: boolean, setShowMigrationAlert: (show: boolean) => void) {
    if (dbConnected) {
  ...
  useEffect(() => {
    if (!dbConnected) {
      setReadings(parseLocalStorageReadings(localStorage.getItem("electricityReadings")))
      setTokens(parseLocalStorageTokens(localStorage.getItem("electricityTokens")))
    } else {
  ...
  const handleAddReading = async (forceUpdate = false) => {
    ...
    try {
      ...
      if (dbConnected) {
        ...
        const result = await addElectricityReading(readingValue, forceUpdate)

        if (!result.isUpdate && result.existingReading && !forceUpdate) {
          // Throw error with existing reading info to trigger duplicate alert
          const error = new Error("Duplicate reading exists") as Error & { existingReading?: ElectricityReading }
        ...
        if (result.isUpdate) {
          ...
          setReadings((prev) => prev.map((r) => (r.reading_id === result.reading.reading_id ? result.reading : r)))
          toast({
            title: "Reading Updated",
            description: `${getPeriodFromHour(new Date().getHours())} reading updated to ${readingValue} kWh.`,
          })
        } else {
          // Add new reading
        ...
        if (existingReadingIndex !== -1 && !forceUpdate) {
          ...
          ;(error as Error & { existingReading?: ElectricityReading }).existingReading = readings[existingReadingIndex]
          throw error
        }
  ...
  const handleAddBackdatedReading = async (readingData: Omit<ElectricityReading, "id" | "reading_id">) => {
    try {
      setIsSubmitting(true)
      ...
      toast({
        title: "Error",
        description: "Failed to add backdated reading. Please try again.",
        variant: "destructive",
      })
    } finally {
      setIsSubmitting(false)
  ...
  const handleAddToken = async () => {
    ...
    try {
      ...
      if (dbConnected) {
        // Use server action if database is connected
        const newToken = await addTokenPurchase(Number(tokenUnits), Number(tokenCost))
  ...
  useEffect(() => {
    const calculateTotalUnits = (): number => {
      ...
      return Number(sortedReadings[0].reading) - Number(sortedReadings[sortedReadings.length - 1].reading)
    }
  ...
  return (
    <div className="grid gap-6">
      ...
      {missedReadings.length > 0 && <MissedReadings missedReadings={missedReadings} />}
        ...
        <CardContent>
          <div className="grid gap-6">
            ...
            <div className="grid gap-4">
              ...
              />
              {!notificationsEnabled && <NotificationsAlert enableNotifications={enableNotifications} />}
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <TabsContent value="backdated">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Record a reading for a specific date and time (e.g., missed readings)</CardDescription>
            </CardHeader>
            <CardContent>
              <BackdatedReadingForm onSubmit={handleAddBackdatedReading} isSubmitting={isSubmitting} />
            </CardContent>
          </Card>
        </TabsContent>
