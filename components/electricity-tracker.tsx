...
import { Bell, Info, Zap, Database, BarChart3, History, CalendarClock } from "lucide-react"
      ...
      if (savedReadings) {
        try {
          ...
          const formattedReadings: ElectricityReading[] = parsedReadings.map((r) => ({
            id: typeof r.id === "number" ? r.id : Number.parseInt(r.id as string) || Date.now(),
      ...
      if (savedTokens) {
        try {
          ...
          const formattedTokens: TokenPurchase[] = parsedTokens.map((t) => ({
            id: typeof t.id === "number" ? t.id : Number.parseInt(t.id as string) || Date.now(),
  ...
  const handleAddBackdatedReading = async (readingData: Omit<ElectricityReading, "id" | "reading_id">) => {
    try {
      ...
      toast({
        ...
        description: `Reading of ${readingData.reading} kWh has been recorded for ${readingData.timestamp.toLocaleString()}.`,
      })
    } catch (error) {
          ...
          <div className="grid gap-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="bg-blue-50 dark:bg-blue-950 p-4 rounded-lg">
                ...
                <p className="text-2xl font-bold">{totalUnits.toFixed(2)} kWh</p>
              </div>
