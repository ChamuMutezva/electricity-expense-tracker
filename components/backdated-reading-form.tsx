...
interface BackdatedReadingFormProps {
  onSubmit: (reading: Omit<ElectricityReading, "id" | "reading_id">) => Promise<void>
  isSubmitting: boolean
}

export default function BackdatedReadingForm({ onSubmit, isSubmitting }: BackdatedReadingFormProps) {
  const [date, setDate] = useState<Date | undefined>(new Date())
  const [time, setTime] = useState("17:00") // Default to 5:00 PM
  const [reading, setReading] = useState("")
  const [period, setPeriod] = useState<"morning" | "evening" | "night">("evening")
