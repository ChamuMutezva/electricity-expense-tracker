...
import { Calendar, TrendingUp, Zap, Clock, ChevronLeft, ChevronRight } from "lucide-react"
...
export default function EnhancedUsageCharts({ readings }: EnhancedUsageChartsProps) {
  ...
  const processedData = useMemo(() => {
    ...
    dates.forEach((date, index) => {
      const dayReadings = readingsByDay[date].sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime())
      ...
      for (let i = 1; i < dayReadings.length; i++) {
        ...
        const consumption = Math.max(0, prevReading.reading - currentReading.reading)

        if (currentReading.period === "morning") morningConsumption += consumption
        else if (currentReading.period === "evening") eveningConsumption += consumption
        else if (currentReading.period === "night") nightConsumption += consumption
      }
      ...
      if (index > 0) {
        ...
        if (prevDayReadings.length > 0 && dayReadings.length > 0) {
          const lastPrevReading = prevDayReadings[prevDayReadings.length - 1]
          const firstCurrentReading = dayReadings[0]
          const overnightConsumption = Math.max(0, lastPrevReading.reading - firstCurrentReading.reading)

          if (firstCurrentReading.period === "morning") morningConsumption += overnightConsumption
        }
      }

      const dateObj = new Date(date)
      const total = morningConsumption + eveningConsumption + nightConsumption
    ...
    return [
      {
        ...
        percentage: grandTotal > 0 ? Number(((totals.morning / grandTotal) * 100).toFixed(1)) : 0,
        color: PERIOD_COLORS.morning,
      },
      {
        period: "Evening",
        consumption: Number(totals.evening.toFixed(2)),
        percentage: grandTotal > 0 ? Number(((totals.evening / grandTotal) * 100).toFixed(1)) : 0,
        color: PERIOD_COLORS.evening,
      },
      {
        period: "Night",
        consumption: Number(totals.night.toFixed(2)),
        percentage: grandTotal > 0 ? Number(((totals.night / grandTotal) * 100).toFixed(1)) : 0,
  ...
  const formatCurrentPeriod = (): string => {
    if (timePeriod === "weekly") {
      const weekEnd = new Date(currentWeekStart)
      weekEnd.setDate(weekEnd.getDate() + 6)
      return `${currentWeekStart.toLocaleDateString(undefined, { month: "short", day: "numeric" })} - ${weekEnd.toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" })}`
    } else if (timePeriod === "monthly") {
      return currentWeekStart.toLocaleDateString(undefined, { year: "numeric", month: "long" })
    }
    return "All Data"
  }
  ...
  if (readings.length < 2) {
    return (
      <div className="text-center py-8 text-muted-foreground">
        ...
        <p className="text-sm">Add at least two readings to see your consumption patterns.</p>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950 dark:to-indigo-950 rounded-xl border">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-blue-900 dark:text-blue-100">View Options</span>
          </div>
      ...
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>Daily consumption patterns across different periods</CardDescription>
        </CardHeader>
        <CardContent>
          <ChartContainer config={chartConfig} className="h-[400px] w-full">
            {chartType === "area" && (
              <AreaChart data={filteredData}>
                ...
                <XAxis dataKey="formattedDate" tick={{ fontSize: 12 }} tickLine={{ stroke: "#e2e8f0" }} />
                <YAxis
                  tick={{ fontSize: 12 }}
            ...
            {chartType === "line" && (
              <LineChart data={filteredData}>
                ...
                <Line
                  ...
                  stroke={PERIOD_COLORS.morning}
                  strokeWidth={3}
                  dot={{ fill: PERIOD_COLORS.morning, strokeWidth: 2, r: 4 }}
                />
                <Line
                  type="monotone"
                  dataKey="evening"
                  stroke={PERIOD_COLORS.evening}
                  strokeWidth={3}
                  dot={{ fill: PERIOD_COLORS.evening, strokeWidth: 2, r: 4 }}
                />
                <Line
                  type="monotone"
                  dataKey="night"
                  stroke={PERIOD_COLORS.night}
                  strokeWidth={3}
                  dot={{ fill: PERIOD_COLORS.night, strokeWidth: 2, r: 4 }}
                />
                <Line
                  type="monotone"
                  dataKey="total"
            ...
            {chartType === "bar" && (
              <BarChart data={filteredData}>
                ...
                <ChartTooltip content={<ChartTooltipContent />} />
                <ChartLegend content={<ChartLegendContent />} />
                <Bar dataKey="morning" fill={PERIOD_COLORS.morning} radius={[0, 0, 4, 4]} />
                <Bar dataKey="evening" fill={PERIOD_COLORS.evening} radius={[0, 0, 4, 4]} />
                <Bar dataKey="night" fill={PERIOD_COLORS.night} radius={[4, 4, 0, 0]} />
              </BarChart>
            )}
            ...
            {chartType === "composed" && (
              <ComposedChart data={filteredData}>
                ...
                <ChartTooltip content={<ChartTooltipContent />} />
                <ChartLegend content={<ChartLegendContent />} />
                <Bar dataKey="morning" fill={PERIOD_COLORS.morning} radius={[2, 2, 0, 0]} />
                <Bar dataKey="evening" fill={PERIOD_COLORS.evening} radius={[2, 2, 0, 0]} />
                <Bar dataKey="night" fill={PERIOD_COLORS.night} radius={[2, 2, 0, 0]} />
                <Line
                  type="monotone"
                  dataKey="total"
                  stroke="#ef4444"
            ...
            {chartType === "radial" && (
              <RadialBarChart data={periodStats} innerRadius="30%" outerRadius="90%">
                <RadialBar dataKey="consumption" cornerRadius={10} fill={(entry) => entry.color} />
                <ChartTooltip content={<ChartTooltipContent />} />
                <ChartLegend content={<ChartLegendContent />} />
              </RadialBarChart>
            )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {periodStats.map((stat, index) => (
          <Card key={stat.period} className="relative overflow-hidden">
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className="p-2 rounded-lg" style={{ backgroundColor: `${stat.color}20` }}>
                    <Clock className="h-5 w-5" style={{ color: stat.color }} />
                  </div>
                  <div>
                    <h3 className="font-semibold text-gray-900 dark:text-gray-100">{stat.period}</h3>
      ...
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>Distribution of electricity usage across different time periods</CardDescription>
        </CardHeader>
        <CardContent>
