...
export default function UsageChart({ readings }: Readonly<UsageChartProps>) {
  ...
  useEffect(() => {
      ...
      for (let i = 1; i < dayReadings.length; i++) {
        ...
        if (prevValue >= currentValue) {
          consumption.push({
            ...
            consumption: Number((prevValue - currentValue).toFixed(2)),
    ...
    for (let i = 1; i < days.length; i++) {
      ...
      if (prevDayReadings.length > 0 && currentDayReadings.length > 0) {
        const lastReadingPrevDay = prevDayReadings[prevDayReadings.length - 1]
    ...
    // Sort by timestamp
    consumption.sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime())

    setDailyConsumption(consumption)

    // Set current week to the most recent week
    if (consumption.length > 0) {
      const latestDate = new Date(consumption[consumption.length - 1].date)
  ...
  useEffect(() => {
    if (!canvasRef.current || filteredData.length === 0 || chartWidth === 0) return
...
*/
    ...
    const availableWidth = chartInnerWidth - 20 // Leave 20px margin on each side
    const dayWidth = availableWidth / totalDays
    const maxBarWidth = Math.min(25, dayWidth * 0.35) // Smaller bars, max 25px
    const periodSpacing = maxBarWidth * 1 // Spacing between periods within a day

    filteredData.forEach((item) => {
      const dayIndex = days.indexOf(item.date)

      // Calculate day center position with proper margins
      const dayCenter = padding.left + 20 + dayIndex * dayWidth + dayWidth / 2
      ...
      const normalizedValue = range === 0 ? 0.5 : (item.consumption - minConsumption) / range
      ...
      const barRight = Math.min(chartWidth - padding.right - 10, x + maxBarWidth / 2)
      ...
      ctx.fillRect(barLeft, chartHeight - padding.bottom - barHeight, actualBarWidth, barHeight)
      ...
      if (barHeight > 20 && actualBarWidth > 15) {
        ...
        const textWidth = ctx.measureText(item.consumption.toFixed(1)).width
        const textX = barLeft + actualBarWidth / 2

        ctx.fillStyle = "rgba(255, 255, 255, 0.9)"
        ctx.fillRect(textX - textWidth / 2 - 2, chartHeight - padding.bottom - barHeight - 18, textWidth + 4, 14)

        ctx.fillStyle = "#1e293b"
        ctx.fillText(item.consumption.toFixed(1), textX, chartHeight - padding.bottom - barHeight - 6)
      }
    })
    ...
    days.forEach((day, i) => {
      ...
      ctx.fillText(formattedDate, dayCenter, chartHeight - padding.bottom + 15)
    })
    ...
    periods.forEach((item, index) => {
      ...
      const legendGradient = ctx.createLinearGradient(0, legendY, 0, legendY + 12)
  ...
  const getEnhancedColorsForPeriod = (period: "morning" | "evening" | "night") => {
    switch (period) {
      case "morning":
        return {
  ...
  const getAverageByPeriod = (period: "morning" | "evening" | "night"): number => {
    const periodConsumption = filteredData.filter((item) => item.period === period)
    if (periodConsumption.length === 0) return 0

    const total = periodConsumption.reduce((sum, item) => sum + item.consumption, 0)
  ...
  if (dailyConsumption.length === 0) {
    return (
      <div className="text-center py-8 text-muted-foreground">
        No consumption data available. Make sure you have multiple readings within the same day or consecutive days.
      </div>
    )
  }
        ...
        <div className="flex items-center gap-3">
          ...
          <Select value={timePeriod} onValueChange={(value) => setTimePeriod(value as TimePeriod)}>
            ...
            <SelectContent className="shadow-lg">
              ...
              <SelectItem value="monthly" className="hover:bg-slate-50">
                📊 Monthly View
              </SelectItem>
        ...
        <Card className="border-emerald-200 bg-gradient-to-br shadow-sm hover:shadow-md transition-shadow">
          <CardContent className="p-6">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="text-sm font-medium text-emerald-800">Morning Usage</div>
            </div>
            <div className="text-2xl font-bold text-emerald-700 mb-1">{morningAvg} kWh</div>
            <div className="text-sm text-emerald-600">Average at 7:00 AM</div>
          </CardContent>
        </Card>
        ...
        <Card className="border-amber-200 shadow-sm hover:shadow-md transition-shadow">
          <CardContent className="p-6">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="text-sm font-medium text-amber-800">Evening Usage</div>
            </div>
            <div className="text-2xl font-bold text-amber-700 mb-1">{eveningAvg} kWh</div>
            <div className="text-sm text-amber-600">Average at 5:00 PM</div>
          </CardContent>
        </Card>
        ...
        <Card className="border-indigo-200 shadow-sm hover:shadow-md transition-shadow">
          <CardContent className="p-6">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="text-sm font-medium text-indigo-800">Night Usage</div>
            </div>
            <div className="text-2xl font-bold text-indigo-700 mb-1">{nightAvg} kWh</div>
            <div className="text-sm text-indigo-600">Average at 9:00 PM</div>
          </CardContent>
        </Card>
      </div>
    </div>
